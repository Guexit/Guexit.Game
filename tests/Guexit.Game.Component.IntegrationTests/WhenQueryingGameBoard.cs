...
public sealed class WhenQueryingGameRoomVoting : ComponentTest
{
    ...
    {
        var gameRoomId = new GameRoomId(Guid.NewGuid());
        var playerId1 = new PlayerId("rocket");
        var playerId2 = new PlayerId("starlord");
        var playerId3 = new PlayerId("wroot");
        ...
        var gameRoom = GameRoomBuilder.CreateStarted(gameRoomId, playerId1, new[] { playerId2, playerId3 })
            ...
            .WithGuessingPlayerThatSubmittedCard(playerId2, playerId3)
            .Build();
        await Save(gameRoom);
        await Save(new[]
        {
            new PlayerBuilder().WithId(playerId1).WithUsername("rocket").Build(),
            new PlayerBuilder().WithId(playerId2).WithUsername("starlord").Build(),
            new PlayerBuilder().WithId(playerId3).WithUsername("wroot").Build()
        });
